...
dump_workload(){
    ...
    WORKLOADS=($(list_names ${NAMESPACE} ${WORKLOAD_NAME}))
...
if [ -z ${NAMESPACE} ];then
WORKLOAD_NAMES=($(kubectl api-resources -oname --namespaced=false | grep -vE "(componentstatuses|authentication.k8s.io|authorization.k8s.io)"))
NAMESPACE="default"
else
WORKLOAD_NAMES=($(kubectl api-resources -oname --namespaced=true | grep -vE "(bindings|secrets|authorization.k8s.io)"))
fi
